# VDA5050 Master + Client — Design Doc
...
code. For just running the demo, see `../README.md` instead.
...
with a rendered PNG next to it (regenerate from the repo root via `java
-jar .tools/plantuml.jar "docs/diagrams/*.plantuml"` after editing any
of them).
...
- `master.py` derives `OrderPhase` per AGV from `State` the same way the
  ...
  five-value phase the reference design uses. One deliberate deviation:
  their `OrderStatus.msg` prefixes each value (`PHASE_NO_ORDER`,
  `PHASE_ACCEPTED`, …) since flat ROS `uint8` constants need it to avoid
  clashing; our `OrderPhase` enum drops the prefix since the enum class
  already disambiguates (`OrderPhase.NO_ORDER`) — same values, shorter
  names.
...
```
vda5050_mvp/
  README.md                   # quickstart — run the demo
  ...
  docs/
    design_doc.md              # this file — the architecture/design doc
